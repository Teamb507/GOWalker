...
public:
    ...
    {
        sync_buffer();
        if (max_vert > curr_vert)
            sync_zeros(max_vert - curr_vert);
        flush_buffer();
    ...
    if (reprocessed)
    {
        ...
        while (fgets(line, 1024, fp) != NULL)
        {
            ...
            if (outdegree.find(from) != outdegree.end())
            {
                ...
                outdegree.emplace(from, 1);
            if (indegree.find(to) != indegree.end())
            {
                indegree[to]++;
            }
            else
                indegree.emplace(to, 1);
#endif
            if (from == to)
                continue;
...
void test(std::string file, uint64_t nedges)
{
    std::ifstream input(file);
    uint32_t ver;
    uint64_t num = 0;
    while (input.read((char *)&ver, 4))
    {
    ...
    if (num != nedges)
    {
        std::cout << "csr file error" << std::endl;
    }
}

size_t metis_convert(std::string filename, std::string partfile, graph_converter &converter, bool skip = false)
{
    std::string base_name = remove_extension(filename);
    bool reprocessed = true;
    if (test_dataset_processed_exists(base_name))
    ...
    vid_t min_vert;
    load_graph_meta(base_name, &nvertices, &nedges, &min_vert, false);
    test("/home/wyt/Dataset/soc_metis_edge.csr", nedges);
    // 确定块大小
    std::ifstream part(partfile);
    eid_t max = 0;
    std::string s;
    ...
    delete_processed_block_data(base_name, blocksize);
    /* split the data into multiple blocks */
    split_blocks_metis(base_name, partfile, 0);
    if (converter.need_sorted())
    {
        sort_vertex_neighbors(base_name, blocksize, converter.is_weighted());
    }
