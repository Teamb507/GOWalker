...
public:
    ...
    int iRand(int min, int max) { return lRand() % (max - min) + min; }
        inline void inlineRandPair(float max_val, size_t bound, float &rand_val, size_t &rand_pos) {
        uint64_t r = lRand();
        // 高32位作为 int，低32位作为 double
        rand_pos = static_cast<size_t>(r >> 32) % bound;

        const union {
            uint64_t i;
            float d;
        } u = {UINT64_C(0x3FF) << 52 | (r & ((1ULL << 52) - 1))}; // 低52位 → double [1.0, 2.0)
        rand_val = (u.d - 1.0) * max_val;                         // 缩放到 [0, max_val)
    }
};
