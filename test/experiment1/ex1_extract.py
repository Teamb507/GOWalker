...
def extract_all_times(file_content):
    ...
    for match in blocks:
        ...
        # 提取当前块中的时间数据
        total_time = gpu_time = cpu_time = None
        
        for line in block.strip().split('\n'):
            line = line.strip()
            if line.startswith('.0_Total_time='):
                total_time = float(line.split('=')[1])
            elif line.startswith('.1_GPUTime='):
                gpu_time = float(line.split('=')[1])
            elif line.startswith('.2_CPUTime='):
                cpu_time = float(line.split('=')[1])
        
        # 确保提取到了所有三个时间指标
        if total_time is not None and gpu_time is not None and cpu_time is not None:
            all_times.append((total_time, gpu_time, cpu_time))
