...
public:
    ...
    graph_test* test;
    gpu_stream* stream;
    cache_block* d_map; // 零拷贝
    gpu_graph* g_graph;
    gpu_cache* g_cache;
    curandState* states; // 更改gpu的线程配置;
    gpu_walks** g_walks;
    // cpu游走线程池
    ...
    graph_engine(graph_cache& _cache, graph_walk& manager, graph_driver& _driver, graph_config& _conf, metrics& m, graph_cache& _g_cache, scheduler* shcedule) : _m(m)
    {
        ...
        cpu_updatepool = new ThreadPool(conf->cpu_threads, conf->cpu_threads); // 最大线程数，最大任务数（任务数<=线程数）
        ...
        states = cudarand(conf->blockpergrid, conf->threadperblock, stream->graph); // 更改gpu的线程配置;
    ...
    int sidewalkupdate(walker_t& walker, real_t p, real_t q, hid_t maxhop, RandNum* seed, graph_engine* cpu_engine, tid_t tid)
    {
        ...
        //_m.stop_time("2.2.1_Sidewalkupdate");

        // _m.start_time("2.2.2_Walker_makeup");
        if (hop < maxhop)
        {
    ...
    int cggraph_n2_update(walker_t& walker, real_t p, real_t q, hid_t maxhop, RandNum* seed, graph_engine* cpu_engine, tid_t tid, std::vector<bid_t> buckets)
    {
        bid_t nblocks = walk_manager->global_blocks->nblocks;
        vid_t cur_vertex = walker.current, prev_vertex = walker.previous;
        if (cur_vertex != walker.current)
        ...
        while (hop < maxhop)
        {
            ...
            if (std::find(buckets.begin(), buckets.end(), cur_blk) != buckets.end() && std::find(buckets.begin(), buckets.end(), prev_blk) != buckets.end())
            {
                break;
            }
        }
    ...
    int cggraph_SOPR_update(walker_t& walker, real_t alpha, hid_t maxhop, RandNum* seed, graph_engine* cpu_engine, tid_t tid, std::vector<bid_t> buckets)
    {
        bid_t nblocks = walk_manager->global_blocks->nblocks;
        vid_t cur_vertex = walker.current, prev_vertex = walker.previous;
        hid_t hop = walker.hop;
...
#ifdef IO_UTE
        std::cout << "gpu load block:";
        for (int pos = 0; pos < block_scheduler->cachesize(); pos++)
        {
            std::cout << block_scheduler->buckets[pos] << " ";
        }
        std::cout << std::endl;
#endif
    ...
    void cggraph_pipe_run()
    {
        while (walk_manager->nwalks() > 0)
        {
            ...
            if (res)
            {
                ...
                for (bid_t i = 0; i < walk_manager->totblocks; i++)
                {
                    if ((std::find(block_scheduler->buckets.begin(), block_scheduler->buckets.end(), i % nblocks) != block_scheduler->buckets.end()) && (std::find(block_scheduler->buckets.begin(), block_scheduler->buckets.end(), i / nblocks) != block_scheduler->buckets.end()))
                    {
                        for (int tid = 0; tid < conf->cpu_threads; tid++)
                        {
                            if (walk_manager->cpu_walkbuffer[i][tid].bsize > 0)
...
void walkjob(void* arg)
{
    ...
    if (engine->conf->algorithm == node2vec)
    {
        for (wid_t i = 0; i < walknum; i++)
        {
            if (conf->cggraph == true) {
                steps += (uint64_t)engine->cggraph_n2_update(walks[i], p, q, maxhop, seed, engine, tid, buckets);
            }
            else
    ...
    else if (engine->conf->algorithm == SOPR)
    {
        ...
        {
            if (conf->cggraph == true) {
                steps += (uint64_t)engine->cggraph_SOPR_update(walks[i], alpha, maxhop, seed, engine, tid, buckets);
            }
            else
